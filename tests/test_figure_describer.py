"""Figure understanding: crop -> describe (local Qwen2-VL by default, or a cloud LLM) -> 'figure' chunk.
The renderer, the local VLM, and the cloud provider are all mocked, so these tests are fast, offline,
and need no model/key/GPU."""
...
def test_backend_default_is_local(monkeypatch):
    monkeypatch.delenv("FIGURE_BACKEND", raising=False)
    assert fd._backend() == "local"                          # dedicated self-hosted VLM, not a cloud LLM
    monkeypatch.setenv("FIGURE_BACKEND", "llm")
    assert fd._backend() == "llm"
...
def test_reference_text_pulls_the_papers_own_sentences():
    assert "Validation loss" in fd._reference_text(_PARSED, "1")
    assert fd._reference_text(_PARSED, "9") == ""
...
def test_figure_chunks_local_happy_path(monkeypatch):
    monkeypatch.setenv("ENABLE_FIGURE_UNDERSTANDING", "true")
    monkeypatch.setenv("FIGURE_CACHE", "false")
    monkeypatch.delenv("FIGURE_BACKEND", raising=False)      # default: local VLM
    monkeypatch.setattr(fd, "_render_page_png", lambda path, page, px, num=None: b"\x89PNG-bytes")
    monkeypatch.setattr(fd, "_describe_local",
                        lambda cap, ref, png: "It shows loss falling from 2.1 to 0.4 over 50 epochs.")
...
def test_figure_chunks_skips_unrenderable(monkeypatch):
    ...
    monkeypatch.setattr(fd, "_describe_local", lambda cap, ref, png: "desc")
    assert fd.figure_chunks(Path("x.pdf"), _PARSED) == []


def test_figure_chunks_uses_cache_and_skips_the_model(monkeypatch):
    # A cached figure must NOT load/run the model (the describe-once cost win).
    monkeypatch.setenv("ENABLE_FIGURE_UNDERSTANDING", "true")
    ...
    def _boom(*a, **k):
        raise AssertionError("model was called despite a cache hit")

    monkeypatch.setattr(fd, "_describe_local", _boom)
    chunks = fd.figure_chunks(Path("x.pdf"), _PARSED)
    assert len(chunks) == 2 and "cached desc for fig 1" in chunks[0]["text"]


def test_llm_backend_empty_when_no_provider(monkeypatch):
    monkeypatch.setenv("ENABLE_FIGURE_UNDERSTANDING", "true")
    monkeypatch.setenv("FIGURE_BACKEND", "llm")
...
def test_describe_llm_builds_vision_message_and_is_failsafe(monkeypatch):
    monkeypatch.setenv("FIGURE_DESCRIBE_RETRIES", "0")
    ...
    fp = _FakeVision()
    out = fd._describe_llm(fp, "Figure 1: loss vs epochs", "the paper says loss drops", b"PNGDATA")
    assert out == "The chart shows a downward loss curve."
    assert fp.kwargs.get("timeout", 0) >= 5
    content = fp.messages[0]["content"]
    assert {part["type"] for part in content} == {"text", "image_url"}
    text_part = next(p for p in content if p["type"] == "text")["text"]
    assert "the paper says loss drops" in text_part
    ...
    assert fd._describe_llm(_Boom(), "cap", "ref", b"x") == ""
